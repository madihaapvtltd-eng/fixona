  ...
  const fetchProject = async () => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        setDepartmentWorks(data.departmentBreakdown?.length ? data.departmentBreakdown.map((w: any) => {
          // Handle backward compatibility: if old structure (description, estimatedCost), convert to new task structure
          if (w.description && !w.tasks) {
            return {
              ...
              tasks: [{
                ...
                hasCost: !!w.estimatedCost,
                status: 'pending'
              }],
              isThirdParty: w.isThirdParty || false
            };
          }
